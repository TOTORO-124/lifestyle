...
export default function OneCard({ session, currentUser, onLeave }: { session: Session; currentUser: any; onLeave?: () => void }) {
  ...
  return (
    <div className={`w-full h-full flex flex-col bg-slate-900 text-slate-100 relative overflow-hidden font-sans ${shake ? 'animate-[shake_0.5s_ease-in-out]' : ''}`}>
      ...
      <div className="flex-none flex justify-center items-start pt-6 px-4 gap-3 sm:gap-8 overflow-x-auto custom-scrollbar min-h-[140px] z-10">
        {opponentIds.map((pid) => {
          ...
          return (
            <div key={pid} className={`relative flex flex-col items-center bg-slate-800/60 backdrop-blur-sm p-3 sm:p-5 rounded-2xl border-2 transition-all duration-300 ${isCurrent ? 'border-indigo-500 shadow-[0_0_20px_rgba(99,102,241,0.4)] -translate-y-2' : 'border-slate-700/50'}`}>
      ...
      {!isSpectator && myState && (
        <div className="flex-none bg-slate-800/90 backdrop-blur-xl rounded-t-[2rem] border-t border-slate-700/80 p-4 sm:p-6 flex flex-col items-center pb-6 sm:pb-8 shadow-[0_-10px_40px_rgba(0,0,0,0.3)] z-30">
          ...
          <>
          <div className="flex flex-col sm:flex-row items-center justify-between w-full max-w-5xl mb-4 sm:mb-6 gap-3 relative">
