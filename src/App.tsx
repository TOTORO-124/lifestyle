          ...
          <h1 className="text-xs sm:text-sm font-bold tracking-tight truncate max-w-[120px] sm:max-w-none flex items-center gap-2">
            <span>
              {session.gameType === GameType.LIAR ? '오피스_라이어_시트.xlsx' : 
               ...
               session.gameType === GameType.YUT_NORI ? '전통_윷놀이_시뮬레이션.xlsx' : '데이터_무결성_검증.xlsx'}
            </span>
                      ...
                      {session.gameType === GameType.YUT_NORI && (
                        <div className="space-y-4">
                          <div className="flex items-center justify-between p-3 bg-gray-50 rounded border border-[#d1d1d1]">
                            ...
                            <div className="flex gap-2">
                              <button
                                onClick={() => sessionService.updateSettings(session.id, { yutNoriMode: 'INDIVIDUAL' })}
                                className={`px-3 py-1 rounded text-[10px] font-bold border ${session.settings.yutNoriMode !== 'TEAM' ? 'bg-[#217346] text-white border-[#217346]' : 'bg-white text-gray-600 border-gray-300'}`}
                              >
                                개인전
                              </button>
                              <button
                                onClick={() => sessionService.updateSettings(session.id, { yutNoriMode: 'TEAM' })}
                                className={`px-3 py-1 rounded text-[10px] font-bold border ${session.settings.yutNoriMode === 'TEAM' ? 'bg-[#217346] text-white border-[#217346]' : 'bg-white text-gray-600 border-gray-300'}`}
                              >
                                팀전
                              </button>
          ...
          {session.status === SessionStatus.PLAYING && (
            session.gameType === GameType.OMOK ? (
              ...
              <SuikaGame 
                ...
                onGameOver={(score) => {
                  sessionService.updateSuikaStats(session.id, currentUser.uid, score);
                  ...
                  <div className="text-center space-y-4">
                    ...
                    <div className="py-6 border-y border-[#d1d1d1] space-y-4">
                      {session.gameType === GameType.DRAW ? (
                        ...
                        <div className="space-y-4">
                          <div className="text-xs text-[#666]">최종 스코어:</div>
                          <div className="text-4xl font-black text-[#217346]">
                            {session.suikaGame?.score || 0}점
                          </div>
                          <div className="text-sm text-[#666]">
                            최고 기록: {session.suikaGame?.bestScore || 0}점
                          </div>
                        </div>
                      ) : session.gameType === GameType.BINGO ? (
                        <div className="space-y-4">
